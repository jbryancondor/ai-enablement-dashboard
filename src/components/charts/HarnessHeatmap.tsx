      ...
      <div>
        <div style={{
          ...
          {data.map((row, rowIdx) => {
            ...
            const rowDelay = rowIdx * 65;
            return [
              <div key={`dot-${row.path}`} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: CELL_H, animation: 'staggerFadeIn 0.6s ease both', animationDelay: `${rowDelay}ms` }}>
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: TIER_DOTS[row.tier], display: 'inline-block' }} title={row.tier} />
              </div>,

              <div key={`name-${row.path}`} title={`${row.path} · tier: ${row.tier}`} style={{ height: CELL_H, display: 'flex', alignItems: 'center', paddingRight: 8, overflow: 'hidden', animation: 'staggerFadeIn 0.6s ease both', animationDelay: `${rowDelay}ms` }}>
